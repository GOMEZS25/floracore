...
CREATE TABLE `lotes` (
    ...
    `zona_corte` VARCHAR(80) NULL,
    `notas` VARCHAR(200) NULL,
    `created_by` BIGINT NOT NULL,
    `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),

    UNIQUE INDEX `lotes_numero_lote_key`(`numero_lote`),
    INDEX `lotes_product_id_idx`(`product_id`),
    INDEX `lotes_location_id_idx`(`location_id`),
    INDEX `lotes_estado_idx`(`estado`),
    INDEX `lotes_fecha_ingreso_idx`(`fecha_ingreso`),
    PRIMARY KEY (`lote_id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `stock_movements` (
    ...
    INDEX `stock_movements_lote_id_idx`(`lote_id`),
...
-- AddForeignKey
ALTER TABLE `products` ADD CONSTRAINT `products_packaging_id_fkey` FOREIGN KEY (`packaging_id`) REFERENCES `packaging`(`packaging_id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `products` ADD CONSTRAINT `products_category_id_fkey` FOREIGN KEY (`category_id`) REFERENCES `categories`(`category_id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `lotes` ADD CONSTRAINT `lotes_product_id_fkey` FOREIGN KEY (`product_id`) REFERENCES `products`(`product_id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `lotes` ADD CONSTRAINT `lotes_location_id_fkey` FOREIGN KEY (`location_id`) REFERENCES `inventory_locations`(`location_id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `lotes` ADD CONSTRAINT `lotes_created_by_fkey` FOREIGN KEY (`created_by`) REFERENCES `users`(`user_id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `stock_movements` ADD CONSTRAINT `stock_movements_lote_id_fkey` FOREIGN KEY (`lote_id`) REFERENCES `lotes`(`lote_id`) ON DELETE RESTRICT ON UPDATE CASCADE;
